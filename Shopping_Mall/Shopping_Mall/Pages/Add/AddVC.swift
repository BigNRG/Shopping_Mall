...
class AddVC: UIViewController {
    ...
    @IBOutlet weak var paymentTimeDropDown: DropDown!
...
//        }
    ...
    func setupDropDown() {
        ...
        regionDropDown.didSelect{(selectedText ,index ,id) in
            ...
            self.cityDropDown.optionArray = [""]
            self.cityDropDown.text = ""
            LocationRequestManager.getCity(culture: "EN", regionID: index + 1) { [weak self] loadedCity in
                self?.cityDropDown.optionArray = self?.getCities(loadedCity: loadedCity ?? []) ?? [""]
                self?.cityIdArray = self?.getCityIds(loadedCity: loadedCity ?? []) ?? []
            }
            
        }
        
        cityDropDown.didSelect(completion: { (selectedText, index, id)  in
            print("Selected City: \(selectedText) \n index: \(index)")
            self.realEstate.updateValue(self.cityIdArray[index], forKey: "city")
        })
        
        typeDropDown.optionArray = type
        typeDropDown.didSelect(completion: { (selected, index, id)  in
            print("Selected Type: \(selected) \n index: \(index)")
            self.realEstate.updateValue(index, forKey: "aim")
        })
        
        stateDropDown.optionArray = state
        stateDropDown.didSelect(completion: { (selected, index, id)  in
            print("Selected State: \(selected) \n index: \(index)")
            self.realEstate.updateValue(index, forKey: "state")
        })
        
        paymentTimeDropDown.optionArray = paymentTime
        paymentTimeDropDown.didSelect(completion: { (selected, index, id)  in
    ...
    @IBAction func addButtonAction(_ sender: Any) {
        ...
        realEstate.updateValue(base64Images, forKey: "imagesList")
        let price = priceTextField.text
        realEstate.updateValue(Int(price!) ?? 0, forKey: "price")
        realEstate.updateValue(descriptionTextView.text ?? "", forKey: "description")
        ...
        AdRequestManager.addRealEstate(realEstateDict: realEstate, culture: "EN") { addedElements in
            print("AddedElements: ", addedElements?.count)
        }
    }
